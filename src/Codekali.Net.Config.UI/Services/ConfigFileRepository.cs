...
/// </summary>
internal sealed class ConfigFileRepository(ConfigUIOptions options, ILogger<ConfigFileRepository> logger) : IConfigFileRepository
{
    private readonly string _configDirectory = options.ConfigDirectory ?? Directory.GetCurrentDirectory();
    ...
    public IEnumerable<string> DiscoverFiles()
    {
        ...
        {
            logger.LogWarning("Config directory does not exist: {Dir}", _configDirectory);
            return Enumerable.Empty<string>();
        }

        return Directory
            .EnumerateFiles(_configDirectory, "appsettings*.json", SearchOption.TopDirectoryOnly)
            .Where(f => !Path.GetFileName(f).Contains(".audit.", StringComparison.OrdinalIgnoreCase)
    ...
    {
        logger.LogDebug("Reading file: {Path}", fullPath);
    ...
    {
        logger.LogDebug("Writing file: {Path}", fullPath);
    ...
    {
        logger.LogDebug("Copying {Source} -> {Destination}", sourcePath, destinationPath);
